...
- `exercise8/` — deploys the exercise 7 images to Minikube, translating each Compose concept to its Kubernetes equivalent (see the table in `README.md`). `kubernetes/` holds a ConfigMap (Postgres init SQL, inlined from `exercise7/db/init.sql`) plus a Deployment+Service pair per service; image tags (`products-api:1.0`, `products-ui:1.0`) must be built **from the `exercise7/products-api` and `exercise7/products-ui` directories** and loaded with `minikube image load` before applying — `exercise8/products-api/`, `exercise8/products-ui/`, and `exercise8/init.sql` are stray duplicates of the exercise 7 sources and are not referenced by the lab, the manifests, or any build command; treat `exercise7/` as the single source of truth for this code and don't edit the exercise8 copies. `products-api`'s readiness probe calls the real `/api/products` endpoint (not a TCP/port check) so the Pod gets no traffic until the full Liberty→JNDI→JDBC→Postgres chain works; its liveness probe deliberately checks only that the port is open, so a database outage doesn't cause Kubernetes to kill and restart otherwise-healthy API pods.
